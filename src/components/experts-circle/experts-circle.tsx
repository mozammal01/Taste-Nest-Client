...
export default function ExpertsCircle({ title, name, signatureImg, img }: { title: string; name: string; signatureImg: string; img: string }) {
  return (
    <div className="relative group">
      ...
      <div className="bg-white dark:bg-slate-900 p-6 md:p-8 rounded-[24px] md:rounded-[32px] md:-mt-24 relative z-10 border-2 border-gray-100 dark:border-slate-800 shadow-xl mx-4 text-center transition-colors duration-300">
        <div className="flex flex-col gap-1 md:gap-2 items-center">
          <p className="text-[10px] md:text-xs font-black uppercase tracking-[0.2em] text-primary">{title}</p>
          <p className="text-xl md:text-2xl font-black text-gray-900 dark:text-white">{name}</p>
